...
//    }
    ...
    default String asCsvText() {
        ...
        //rely on each point to generate the best possible "csv text" of itself.
        points().forEach(pt -> sb.append(pt.asCsvText()));
    ...
    default TimeWindow asTimeWindow() {
        return TimeWindow.enclosingWindow(points());
    }
...
//    }
     ...
     * ordering to speed up time-based searches. Use {@link Track#subset(TimeWindow)}
     * and {@link Track#subset(Instant, Instant) } to take advantage of this
     * optimization.
     *
     ...
     * @return A completely distinct collection of Points.
     */
    default NavigableSet<? extends Point> subset(TimeWindow window) {
        checkNotNull(window);
     ...
     * @return A completely distinct collection of Points.
     */
    default NavigableSet<? extends Point> subset(Instant startTime, Instant endTime) {
        return Points.subset(
            TimeWindow.of(startTime, endTime), points()
        );
    }
