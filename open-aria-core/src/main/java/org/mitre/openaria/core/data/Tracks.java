...
//    }
     ...
     */
    private static double maxDistance(NavigableSet<org.mitre.openaria.core.Point> src, NavigableSet<org.mitre.openaria.core.Point> destination) {

        double maxDist = 0;

        for (org.mitre.openaria.core.Point srcPoint : src) {

            org.mitre.openaria.core.Point ceiling = destination.ceiling(srcPoint);
            org.mitre.openaria.core.Point floor = destination.floor(srcPoint);

            if (ceiling == null || floor == null) {
                continue;
            }

            org.mitre.openaria.core.Point interpolatedPoint = interpolate(floor, ceiling, srcPoint.time());
    ...
    public static Track createTrackFromResource(Class clazz, String fileName) {
        ...
        return createTrackFromCsvFile(trackFile.get());
    }

    /**
     * Create a new Track by parsing all the "ARIA CSV" data found within a single file.
     *
     * @param sourceFile A file containing ARIA CSV data
     *
     * @return A Track that contains the Point found within the given sourceFile
     */
    public static Track createTrackFromCsvFile(File sourceFile) {
...
//            Optional<org.mitre.openaria.core.Point> opt1 = t1.interpolatedPoint(currentTime);
//            Optional<org.mitre.openaria.core.Point> opt2 = t2.interpolatedPoint(currentTime);
//
//            //set the "isClose" variable
//            if (!opt1.isPresent() || !opt2.isPresent()) {
//                isClose = false;
//            } else {
//                org.mitre.openaria.core.Point p1 = opt1.get();
//                Point p2 = opt2.get();
